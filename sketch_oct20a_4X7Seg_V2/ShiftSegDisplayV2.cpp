...
void ShiftSegDisplay::writeShift2x(__uint8_t latchPin, __uint8_t dataPin, __uint8_t clockPin, __uint8_t shiftByte0, __uint8_t shiftByte1){
  ...
  shiftOut(dataPin, clockPin, LSB_FIRST, shiftByte1);//LSB_FIRST , MSB_FIRST //Used 0bAAAAANNN
  shiftOut(dataPin, clockPin, LSB_FIRST, shiftByte0);//LSB_FIRST , MSB_FIRST //Used 0bCCACACAC
  //return the latch pin high to signal chip that it
  //no longer needs to listen for information
  digitalPinWrite(latchPin, HIGH);
  //*/
  ///*
  //Use BetterShiftRegControl
  this->registerSIPO->set_shift_register_outputs(0, shiftByte0);
  this->registerSIPO->set_shift_register_outputs(1, shiftByte1);
  this->registerSIPO->update();
  //*/
};
void ShiftSegDisplay::showL1L2L3Display(bool set_L1, bool set_L2, bool set_L3){
  this->shiftByte0 = 0b00000000;// //Used 0bCCACACAC
  this->shiftByte1 = 0b00000000;// //Used 0bAAAAANNN
  ...
  digitalPinWrite(outputEnablePin, HIGH);
  writeShift2x(latchPin, dataPin, clockPin, shiftByte0, shiftByte1|extraOutPutPins);
  digitalPinWrite(outputEnablePin, LOW);
  delay(1);
};

void ShiftSegDisplay::init()
{
  /*
  IC_P_DS
  IC_P_SH_CP
  ...
  */
  ...
  //this->registerSIPO = new BetterShiftRegControl(2, (IC_P_DS|IC_P_SH_CP|IC_P_ST_CP)|(IC_P_MR * false)|(IC_P_OE * false));
  Serial.println(this->_usablePins,BIN);
  this->registerSIPO = new BetterShiftRegControl(2, this->_usablePins);
  if (this->_usablePins&IC_P_DS){
    this->registerSIPO->setPinID(Data_74HC595, this->dataPin);
  };
  if (this->_usablePins&IC_P_SH_CP){
    this->registerSIPO->setPinID(Clock_74HC595, this->clockPin);
  };
  if (this->_usablePins&IC_P_ST_CP){
    this->registerSIPO->setPinID(Latch_74HC595, this->latchPin);
  };
  if (this->_usablePins&IC_P_MR){
    this->registerSIPO->setPinID(Reset_74HC595, this->resetPin);
  };
  if (this->_usablePins&IC_P_OE){
    this->registerSIPO->setPinID(OutputEnable_74HC595, this->outputEnablePin);
  };
  this->registerSIPO->setShiftOrder( LSBFIRST );

}

void ShiftSegDisplay::begin()
{
  this->registerSIPO->setupPinModes();

  /*
  this->hiddenPinMode(this->registerSIPO->getPinID(Data_74HC595), OUTPUT);
  this->hiddenPinMode(this->registerSIPO->getPinID(Clock_74HC595), OUTPUT);
  this->hiddenPinMode(this->registerSIPO->getPinID(Latch_74HC595), OUTPUT);
  this->hiddenPinMode(this->registerSIPO->getPinID(Reset_74HC595), OUTPUT);
  this->hiddenPinMode(this->registerSIPO->getPinID(OutputEnable_74HC595), OUTPUT);
  */
  this->pinModes_Setup = true;
};
...
void ShiftSegDisplay::showFromChar(char charInput[charInputLength] , int delayrepeat)
{
  ...
  __uint8_t set_extraPins = 0b000;//LOW,LOW,LOW
...
{
  __uint8_t shiftByte0 = 0b00000000;// //Used 0bCCACACAC
  __uint8_t shiftByte1 = 0b00000000;// //Used 0bAAAAANNN
  u_int8_t cSelect = 0b00000000;//|1|2|L1L2L3|3|4|  //Used 11111NNN
  //TrueHIGH , FalseLOW
  bool set_DP = set_DP_;
